    ...
    <>
      ...
      <div className="fixed right-0 top-0 h-full w-full max-w-md bg-white z-50 shadow-xl flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-stone-200">
          <h2 className="text-lg font-semibold text-stone-900">Panier</h2>
          <button onClick={closeCart} className="text-stone-400 hover:text-stone-600">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          {items.length === 0 ? (
            <p className="text-stone-400 text-center mt-12">Votre panier est vide</p>
          ) : (
            <ul className="space-y-4">
              {items.map((item) => (
                <li key={`${item.id}-${item.format}`} className="flex gap-4">
